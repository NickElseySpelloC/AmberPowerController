...
DEFAULT_TARGET_DIR=~/scripts/AmberPowerController
...
REMOVE_FILES_AND_FOLDERS=(
  "PowerController.log"
  "PowerController.sh"
  "PowerControllerRun.csv"
  "SwitchStateDebug.json"
  "utility.py"
)
...
# If PowerControllerConfig.yaml exists rename it to config.yaml
if [ -f "$TARGET_DIR/PowerControllerConfig.yaml" ]; then
  mv "$TARGET_DIR/PowerControllerConfig.yaml" "$TARGET_DIR/config.yaml"
  echo "Renamed PowerControllerConfig.yaml to config.yaml"
fi
...
# If PowerControllerState.json exists rename it to system_state.json
if [ -f "$TARGET_DIR/PowerControllerState.json" ]; then
  mv "$TARGET_DIR/PowerControllerState.json" "$TARGET_DIR/system_state.json"
  echo "Renamed PowerControllerState.json to system_state.json"
fi
