...
def register_logger(logger):
    ...
    """
    global logger_function  # noqa: PLW0603
...
def register_configurator(configurator):
    ...
    """
    global config_object  # noqa: PLW0603
    config_object = configurator
    global config_settings  # noqa: PLW0603
...
def register_scheduler_state(state):
    ...
    """
    global scheduler_state  # noqa: PLW0603
...
def send_email(subject, body):
    ...
    if (
        config_settings["Email"]["EnableEmail"] is None
        or not config_settings["Email"]["EnableEmail"]
    ):
        logger_function(
            f"SMTP settings not fully configured for sending emails. Skipping sending the email {subject}.",
            "debug",
        )
        return
...
def report_fatal_error(message, report_stack=False, calling_function=None):  # noqa: FBT002
    ...
    if function_name != "send_email" and not fatal_error_tracking("get"):
        send_email(
            "PowerController terminated with a fatal error",
...
class ConfigManager:
    ...
    def __init__(self):
        ...
        if not Path(config_file_path).exists():
            with Path(config_file_path).open("w", encoding="utf-8") as file:
    ...
    def validate_no_placeholders(self, config_section, path=""):
        ...
        if isinstance(config_section, dict):
            ...
            print(
                f"ERROR: Config value at '{path}' is still set to placeholder: '{config_section}'",
                file=sys.stderr,
            )
            print(
                f"Please update {CONFIG_FILE} with your actual credentials.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    def get_target_hours(self, for_date):
        ...
        if self.is_no_run_today():
            ...
            target_hours = self.default_config["DeviceRunScheule"][
                "TargetRunHoursPerDay"
            ]
            ...
            if (
                "MonthlyTargetRunHoursPerDay" in self.active_config["DeviceRunScheule"]
                and month
                in self.active_config["DeviceRunScheule"]["MonthlyTargetRunHoursPerDay"]
            ):
                target_hours = self.active_config["DeviceRunScheule"][
                    "MonthlyTargetRunHoursPerDay"
                ][month]
            ...
            if (
                target_hours
                < self.default_config["DeviceRunScheule"]["MinimumRunHoursPerDay"]
            ):
                target_hours = self.default_config["DeviceRunScheule"][
                    "MinimumRunHoursPerDay"
                ]
                if logger_function is not None:
                    logger_function(
                        f"{device_label} target daily run hours for {month} are too short. Resetting to the minimum of {target_hours}",
                        "warning",
                    )
            elif (
                target_hours
                > self.default_config["DeviceRunScheule"]["MaximumRunHoursPerDay"]
            ):
                target_hours = self.default_config["DeviceRunScheule"][
                    "MaximumRunHoursPerDay"
                ]
                if logger_function is not None:
                    logger_function(
                        f"{device_label} target daily run hours for {month} are too long. Resetting to the maximum of {target_hours}",
                        "warning",
                    )
    ...
    def get_status(self):
        ...
        for attempt in range(self.RETRY_COUNT):
            if config_settings["ShellySmartSwitch"]["Model"] == "ShellyEM":
                ...
                config_settings["ShellySmartSwitch"]["Model"] == "ShellyPlus1PM"
                or config_settings["ShellySmartSwitch"]["Model"] == "Shelly1PMG3"
            ):
    ...
    def get_gen1_em_status(self):
        ...
        try:
            response = requests.get(
                url,
                headers=headers,
                timeout=config_settings["ShellySmartSwitch"]["Timeout"],
            )
